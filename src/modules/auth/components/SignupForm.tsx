...
import { validateLogin, validateSignup, validLogin } from '../utils';
...
const SignupForm = (props: Props) => {
  ...
  const renderGender = () => {
    ...
    GENDER.map((item: any) => {
      arrGender.push(<option value={item.value}>{item.label}</option>);
    });
    return arrGender;
  };
  const renderRegion = () => {
    ...
    >
      ...
      <div className="col-md-12">
        <label htmlFor="inputRepeatPassword" className="form-label">
          <FormattedMessage id="Xác nhận mật khẩu" />
        </label>
        <input
          type="password"
          className="form-control"
